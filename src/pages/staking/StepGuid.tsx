                    ...
                    <div className={`grid gap-4`} style={{ gridTemplateColumns: `repeat(${steps.length}, 1fr)` }}>
                        {
                            steps.map((step, index) => (
                                <div className="text-center" key={step.title}>
                                    <div className={`w-12 h-12 ${theme === 'cardano' ? 'bg-blue-500' : 'starknet-gradient'} text-white rounded-full flex items-center justify-center text-lg font-bold mx-auto mb-3`}>{index + 1}</div>
