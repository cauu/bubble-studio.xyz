...
export const BlogCard = (
    ...
    return (
        <article className="post-card bg-white/80 backdrop-blur-sm rounded-3xl p-6 shadow-xl metric-card cursor-pointer hover:shadow-md" onClick={() => onClick?.(post)}>
            <img
            ...
            <div className="flex flex-wrap gap-2 mb-3">
                {
                    tags.map((tag) => {
                        return (
                            <span className="bg-green-100 text-green-800 px-2 py-1 rounded-full text-xs font-bold">{tag}</span>
                        )
                    })
                }
            </div>
