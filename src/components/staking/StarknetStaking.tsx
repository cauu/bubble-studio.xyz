  ...
  return (
    <div className="relative z-10 max-w-6xl mx-auto px-4 md:px-0">
      <section className="relative z-10 py-6 md:py-8 px-2 md:px-6">
        <div className="max-w-4xl mx-auto text-center">
          <div className="flex items-center justify-center">
            <h1 className="text-3xl md:text-5xl mb-2 text-ink flex items-center justify-center">Pao Validator️</h1>
            <ExternalLink
              strokeWidth={2}
              className="w-4 h-4 md:w-5 md:h-5 ml-2 cursor-pointer text-muted hover:text-brand-incana"
              onClick={handleToDetail}
            />
          </div>

          <p className="text-sm md:text-base text-muted leading-relaxed max-w-4xl mx-auto">
      ...
      <section className="grid grid-cols-2 lg:grid-cols-4 gap-3 md:gap-4 mb-6 md:mb-8 mt-2 md:mt-4">
        <MetricCard icon="💰" title={t('metric.totalPledge')} value={numberWithCommas(totalStake)} description="STRK" />
        <MetricCard icon="📈" title={t('metric.estimatedAnnualizedReturn')} value={`${apy}%`} />
        <MetricCard icon="👥" title={t('metric.delegator')} value={`${totalDelegators}`} />
        <MetricCard
          icon="🔄"
      ...
      <div className="text-center px-4">
        <Button variant="primary" size="lg" onClick={handleStake} className="w-full md:w-auto">
